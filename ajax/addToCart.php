...
if($_POST['action'] == 'update_quantity') {
    $quantity = $_POST['qty'];
    $checkOrder = $query->updateQuantity($_POST['product_id'], $user_id, $quantity);
    if($checkOrder == 'product_qty_updated') {
        $data = ['message' => 'product_qty_updated'];
    } else {
        $data = ['message' => 'product_qty_not_updated'];
    }
...
if ($_POST['action'] == 'add_to_wislist') {
    $res = $query->add_to_wishlist($_POST['product_id'], $user_id);
    echo json_encode($res, true);

}

if($_POST['action'] == 'add_to_cart_from_wishlist') {
    $comment = '';
    $quantity = 1;
    $checkOrder = $query->checkOrder($_POST['product_id'], $user_id, $comment, $quantity);
     //$query->removeWishlistByProduct_id($_POST['product_id'], $user_id);
    $checkCurrentOrderCount = $query->checkCurrentOrderCount($user_id);
    if($checkOrder == 'product_already_exist') {
        $data = ['message' => 'product_already_exist', 'product_count' => $checkCurrentOrderCount];
    } else {
        $data = ['message' => 'product_added', 'product_count' => $checkCurrentOrderCount];
    }
    echo json_encode($data, true);
}
...
<?php } 
if ($_POST['action'] == 'order_history') {
    //print_r($_POST);
    $OrderHistory = $query->getUserwiseOrderDetails($_POST['order_id']);
    ...
    <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
            ...
            foreach ($OrderHistory as $row) {
                $product = $query->getProductByid($row->product_id);
                $i++;
            ?>
                <tr>
                    <td><?php echo $i; ?></td>
