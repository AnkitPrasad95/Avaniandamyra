...
$state = $pdo->prepare("select * from tbl_gallery_category where id =?");
...
if(!empty($result->name && $result->id)){
    ...
    header('location: galleryCat.php');
}
...
for( $i=0 ; $i < $total ; $i++ ) {
  ...
  $final_name = date('dmYhis').'-'.$ai_id.'-'.$i.'.'.$ext;


  //Make sure we have a file path
  if ($path_tmp != ""){
    //Setup our new file path
    $newFilePath = '../assets/uploads/gallery/'.$final_name;

    //Upload the file into the temp dir
	if(move_uploaded_file($path_tmp, $newFilePath)) {	
    $statement = $pdo->prepare("INSERT INTO tbl_gallery (file_path, photo_name, p_category_id, created_at) VALUES (?,?,?,?)");
	$statement->execute(array($file_path, $final_name,$_POST['photocat_id'],date('Y-m-d H:i:s')));
...
if(isset($_POST['delete_photo'])) {
	...
	//Unlink the photo
	if($photo!='') {
		unlink('../assets/uploads/gallery/'.$photo);	
	}
...
<section class="content-header">
	...
	<div class="content-header-right backbtns">
		<a href="galleryCat.php" class="btn btn-primary btn-sm">Back</a>
	</div>
</section>
            ...
            <tbody>
            	...
            	foreach ($result as $row) {
	                ...
	                <tr>
	                    ...
	                    <td>
	                    	<img src="<?php echo BASE_URL.$row['file_path'].$row['photo_name']; ?>" width="140">
	                    </td>
	                    
	                   <td>
	                        <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
	                            <input type="hidden" name="photo_id" value="<?php echo $row['photo_id']; ?>">
	                            <button type="submit" class="btn btn-danger btn-xs" onclick="return confirm('Are you sure?')" name="delete_photo"><i class="fa fa-trash"></i></button>
	                        </form>
