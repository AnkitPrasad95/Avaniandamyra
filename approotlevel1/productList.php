...
if(isset($_GET['category']) && isset($_GET['product'])) {
	//if(!empty($_GET['category']) && empty($_GET['product'])) {
		...
		if(!empty($_GET['category']) && empty($_GET['product'])) {
			if($catDetails->parent_category > 0) {
				$query .= "where SUBSTRING_INDEX(categories, ',', 2)='$cat_ids'";
			} else {
				$query .= "where SUBSTRING_INDEX(categories, ',', 1)='$cat_ids'";
			}
		} 
		...
		if(!empty($_GET['category']) && !empty($_GET['product'])) {
			if($catDetails->parent_category > 0) {
				$query .= "where SUBSTRING_INDEX(categories, ',', 2)='$cat_ids' AND name LIKE '%$product%'";
			} else {
				$query .= "where SUBSTRING_INDEX(categories, ',', 1)='$cat_ids' AND name LIKE '%$product%'";
			}
									...
									<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3">
										<span class="img">
											<?php if (!empty($row['thumbnail_image'])) { ?>
												<img class="lazyloadimages img-responsive" src="<?php echo BASE_URL; ?>assets/images/lazyloader.gif" data-src="<?php echo BASE_URL . $row['file_path'] . $row['thumbnail_image']; ?>" data-srcset="<?php echo BASE_URL . $row['file_path'] . $row['thumbnail_image']; ?>" alt="<?php echo $row['name']; ?>" height="50px;">
...
<script>
	...
	//function lazyloader() {
		//lazyloader
		let lazyimages = [].slice.call(document.querySelectorAll("img.lazyloadimages"));
		//debugger;
