...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE tbl_partner SET name=? WHERE id=?");
    		$statement->execute(array($_POST['name'],$_REQUEST['id']));
		} else {

			

			unlink('../assets/uploads/partners/'.$_POST['current_photo']);

			$file_path = "assets/uploads/partners/";
			$final_name = str_replace(' ','-', $_POST['name']).'.'.$ext;

        	move_uploaded_file( $path_tmp, '../assets/uploads/partners/'.$final_name );

        	$statement = $pdo->prepare("UPDATE tbl_partner SET name=?, file_path=?, photo=? WHERE id=?");
