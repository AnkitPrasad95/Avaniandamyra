...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("SELECT * FROM tbl_services WHERE slug=? AND name!=?");
		...
		if($path1 == '') {
			$statement = $pdo->prepare("UPDATE tbl_services SET  name=?, slug=?, description=?, description2=?,   meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
    		$statement->execute(array($_POST['name'],$slug,$_POST['description'],$_POST['description2'],   $_POST['meta_title'], $_POST['meta_keyword'], $_POST['meta_description'], $_REQUEST['id']));
		}
	
		
		if($path1 != '') {
			unlink('../assets/uploads/service/'.$_POST['current_banner']);
        	...
        	$statement = $pdo->prepare("UPDATE tbl_services SET  name=?, slug=?, description=?, description2=?,  file_path=?, banner=?,  meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
    		$statement->execute(array($_POST['name'],$slug,$_POST['description'],$_POST['description2'],$file_path, $final_name1, $_POST['meta_title'],$_POST['meta_keyword'],$_POST['meta_description'],$_REQUEST['id']));
		}
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<textarea class="form-control" name="description2" id="editor2"><?php echo $description2; ?></textarea>
							</div>
						</div>
