...
if(isset($_GET['delete_id'])) {
    $statement = $pdo->prepare("update tbl_orders set status = 0 WHERE id =?");
	$statement->execute(array($_REQUEST['delete_id']));

    $statement = $pdo->prepare("update tbl_order_details set status = 0 WHERE order_id=?");
	$statement->execute(array($_REQUEST['delete_id']));
        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM tbl_orders where status = 1 order by id desc");
            	$statement->execute();
