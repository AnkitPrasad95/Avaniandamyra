...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
        ...
        if($path == '') {
			$statement = $pdo->prepare("UPDATE tbl_gallery_category SET  name=?, slug=?  WHERE id=?");
            $statement->execute(array($_POST['name'], $_POST['slug'], $_REQUEST['id']));
        }

        if($path != '') {

            if(!empty($_POST['current_photo'])) { 
                unlink('../assets/uploads/gallery/'.$_POST['current_photo']);
            }

            $file_path = 'assets/uploads/gallery/';
            $final_name = $_REQUEST['id'].'-news-'.date('dmyhis').'.'.$ext;
            move_uploaded_file( $path_tmp, '../assets/uploads/gallery/'.$final_name );
            
			$statement = $pdo->prepare("UPDATE tbl_gallery_category SET name=?, slug=?, file_path=?, photo=?  WHERE id=?");
            $statement->execute(array($_POST['name'], $_POST['slug'], $file_path, $final_name, $_REQUEST['id']));
        }
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_gallery_category WHERE id=?");
					...
					<div class="box-body">
                        ...
                        <div class="form-group">
							...
							<div class="col-sm-9" style="padding-top:5px">
								<img src="<?php echo BASE_URL.$file_path.$photo; ?>" alt="event Photo" style="width:200px;height:200px;">
							</div>
						</div>
                        <?php } ?>
						<div class="form-group">
